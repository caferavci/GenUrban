...
od_df = pd.read_csv("Merged_Flow.csv", engine="python")
od_df.columns = od_df.columns.str.strip()

# Use the origin, destination, and people columns
od_df["origin_id"] = od_df["origin"]
od_df["destination_id"] = od_df["destination"] 
od_df["Flow"] = od_df["people"]

# Create readable origin/destination names for tooltip
od_df["Origin"] = "Region " + od_df["origin"].astype(str)
od_df["Destination"] = "Region " + od_df["destination"].astype(str)

# Remove self-links (if any)
od_df = od_df[od_df["origin_id"] != od_df["destination_id"]]

print(f"Loaded {len(od_df)} OD pairs from Generated_Flow.csv")
print(f"Columns used: origin, destination, people")
...
def flow_to_color(flow_value):
    # Custom gradient: Yellow → Light Orange → Orange → Red → Deep Red → Dark Wine
    # Breakpoints: 0, 50, 250, 500, 800, 1000+
    ...
    if flow_value <= 50:
        ...
        # Light Orange → Orange
        ratio = (flow_value - 50) / 200
        r = 255
        g = int(200 - ratio * 60)    # 200 → 140
        b = 0
    elif flow_value <= 500:
        # Orange → Red
        ratio = (flow_value - 250) / 250
        r = 255
        g = int(140 - ratio * 140)   # 140 → 0
        b = 0
    elif flow_value <= 800:
        # Red → Deep Red
        ratio = (flow_value - 500) / 300
        r = int(255 - ratio * 75)    # 255 → 180
        g = 0
        b = 0
    elif flow_value <= 1000:
        # Deep Red → Dark Wine
        ratio = (flow_value - 800) / 200
        r = int(180 - ratio * 90)    # 180 → 90
        g = 0
        b = int(ratio * 20)          # 0 → 20
    else:
        # >1000: Dark Wine
