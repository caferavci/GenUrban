...
def flow_to_color(flow_value):
    # Fixed color scale: 0 = yellow, 5000 = dark red
    # Clamp flow value between 0 and 5000
    flow_clamped = max(0, min(flow_value, 4000))
    norm = flow_clamped / 4000  # normalize to 0-1 based on 5000 max
    
    # Color interpolation from yellow (255,255,0) to dark red (139,0,0)
    r = int(255 - norm * (255 - 139))   # from 255 to 139
    g = int(255 * (1 - norm))           # from 255 to 0 (yellow to no yellow)
    b = 0                               # always 0 (no blue)
...
print(f"Color scale: 0 = Yellow (255,255,0), 5000+ = Dark Red (139,0,0)")
print()
